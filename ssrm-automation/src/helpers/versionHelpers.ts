...
const logVersionDecision = (message: string, result: boolean, details?: string): void => {
  ...
  console.log(`${status}: ${message}${detailsMsg}`);
};
...
export const determineBestUpdateVersion = (
  ...
  console.log(`VERSION CHECK | Current=${currentVersion} | Branch=${isDevBranch ? 'DEV' : 'STABLE'}`);
  console.log(`Available versions: Stable=${latestStableVersion || 'N/A'}, Beta=${latestBetaVersion || 'N/A'}`);
  ...
  if (latestStableVersion && latestBetaVersion) {
    ...
    console.log(`Comparing base versions: Stable ${formatVersionComparison(latestStableVersion, latestBetaVersion, stableVsBetaBase)} Beta`);
    ...
    if (!isCurrentBeta && isDevBranch) {
      ...
      console.log(`Stable user on dev branch - Stable: ${formatVersionComparison(latestStableVersion, currentVersion, stableComparison)}, Beta: ${formatVersionComparison(latestBetaVersion, currentVersion, betaComparison)}`);
    ...
    if (isCurrentBeta) {
      ...
      console.log(`Beta user - comparing base versions: Stable ${formatVersionComparison(latestStableVersion, currentVersion, stableVsCurrentBase)} Current`);
