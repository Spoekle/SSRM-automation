...
const SplashScreen: React.FC<SplashScreenProps> = ({ appVersion, forceVersionCheck = false }) => {
  ...
  useEffect(() => {
    const loadLocalStorageData = () => {
      ...
      try {
        ...
        console.error("Error parsing card config:", error);
      }
  ...
  const checkForUpdates = async () => {
    try {
      console.log('SplashScreen: Checking for updates...');
      console.log(`SplashScreen: Using ${isDevBranch ? 'development' : 'stable'} branch for updates`);
      ...
      console.log(`SplashScreen: Found versions - Stable=${latestStableVersion}, Beta=${latestBetaVersion}`);
      ...
      if (!targetRelease) {
        console.error('SplashScreen: No suitable release found');
        return false;
      }
      ...
      console.error('SplashScreen: Error checking for updates:', error);
      return false;
    }
  };
  ...
  const handleSkipUpdate = () => {
    ...
    console.log("Update skipped, setting skip flag");
    proceedToMainApp();
  };
    ...
    const loadEverything = async () => {
      ...
      try {
        ...
        if (hasUpdate && !skipUpdateCheck) {
          console.log("Update available and not skipped, showing prompt");
          setShowUpdatePrompt(true);
        } else {
          if (hasUpdate) {
            console.log("Update available but skipped");
          }
          proceedToMainApp();
        }
      } catch (error) {
