  ...
  useEffect(() => {
    if (!isVersionLoading && latestVersion) {
      try {
        ...
        console.log(`Footer: Version check - Current=${current}, Latest=${latest}, Branch=${isDevBranch ? 'dev' : 'stable'}`);
        ...
        if (hasSwitchedBranches) {
          console.log(`Footer: Branch switch detected - ${lastUsedBranch} to ${currentBranch}`);
          setHasUpdate(true);
          return;
        }
        ...
        console.log(`Footer: ${statusEmoji} Update status - ${updateInfo.shouldUpdate ? 'Update available' : 'No update needed'}`);
        if (updateInfo.shouldUpdate) {
          console.log(`Footer: Update recommendation - ${updateInfo.updateToVersion} (${updateInfo.useStable ? 'stable' : 'beta'})`);
          console.log(`Footer: Reason - ${updateInfo.reason}`);
        }
      } catch (error) {
