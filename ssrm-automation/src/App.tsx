...
export default function App() {
  ...
  const getLatestVersion = async () => {
    try {
      ...
      setIsLoading(true);
      console.log(`Fetching latest version from GitHub for ${isDevBranch ? 'development' : 'stable'} branch...`);

      // Use Tauri command instead of direct axios call
      const releases: GitHubRelease[] = await invoke('get_github_releases');

      // Store which branch we last used
      localStorage.setItem('lastUsedBranch', isDevBranch ? 'dev' : 'stable');

      // Find the latest stable release
      const stableRelease = releases.find((release: GitHubRelease) => !release.prerelease);
      ...
      if (isDevBranch) {
        const preRelease = releases.find((release: GitHubRelease) => release.prerelease);
