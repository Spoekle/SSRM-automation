...
import { FaDownload, FaImage, FaSearch, FaStar } from 'react-icons/fa';
...
const SSRMThumbnail: React.FC = () => {
    ...
    const downloadThumbnail = async () => {
        if (!imageSrc) return;
        const filename = `SSRM - ${mapInfo?.metadata?.songName || 'Thumbnail'} - ${mapInfo?.metadata?.songAuthorName || 'Author'} - ${chosenDiff}.png`;
        const safeFilename = filename.replace(/[<>:"/\\|?*]/g, '_');

        const success = await nativeDialog.saveImage(imageSrc, safeFilename);
        if (success) {
            createAlert('Saved SSRM thumbnail!', 'success');
        }
    };
        ...
        <div className='w-full min-h-full relative p-4 pt-6 overflow-x-hidden custom-scrollbar'>
            ...
            >
                <motion.div className='text-center mb-4' variants={fadeIn} custom={0}>
                    <div className="flex items-center justify-center gap-3 mb-1">
                        ...
                        <h1 className='text-2xl font-bold'>SSRM Thumbnail Generator</h1>
                    </div>
                    <p className='text-sm mb-3 text-neutral-600 dark:text-neutral-400'>
                        Generate SSRM thumbnails!
                    </p>
                    <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                    >
                        <FaStar size={16} className="text-orange-500 group-hover:scale-110 transition-transform" />
                        <span>Generate SSRM Thumbnail</span>
                    </motion.button>
                </motion.div>
                ...
                {imageSrc && (
                    ...
                    >
                        ...
                        >
                            <div className="absolute inset-0 bg-orange-500 opacity-0 group-hover:opacity-10 dark:group-hover:opacity-20 rounded-xl transition-opacity duration-300" />
                            <motion.div
            ...
            {thumbnailFormModal && (
                <SSRMThumbnailForm
                    ...
                    setThumbnailFormModal={(val: string) => setThumbnailFormModal(val === "ssrm")}
